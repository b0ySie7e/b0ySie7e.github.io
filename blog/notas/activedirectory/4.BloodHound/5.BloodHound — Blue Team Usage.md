...
```
Domain Controllers  → DOMAIN CONTROLLERS@DOMAIN.LOCAL
Domain Admins Group → DOMAIN ADMINS@DOMAIN.LOCAL
Crown Jewels        → SRV01.DOMAIN.LOCAL (servidores críticos)
```
...
python PlumHound.py -p Password123 -ap "DOMAIN USERS@DOMAIN.LOCAL" "WS01.DOMAIN.LOCAL"

# Otro ejemplo
python PlumHound.py -p Password123 -ap "DAVID@DOMAIN.LOCAL" "DOMAIN ADMINS@DOMAIN.LOCAL"
```

La salida indica el edge y los nodos involucrados que hay que modificar para cortar el camino. Por ejemplo:

```
Removing the relationship GpLink between SCREENSAVER@DOMAIN.LOCAL and WORKSTATIONS@DOMAIN.LOCAL breaks the path!
```
