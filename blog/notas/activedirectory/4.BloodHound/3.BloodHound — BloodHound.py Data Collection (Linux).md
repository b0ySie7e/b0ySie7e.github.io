...
bloodhound-python -d domain.local -c DCOnly -u user -p password! -ns 10.129.204.207

# Con nameserver alternativo (cuando el DNS no apunta al DC)
bloodhound-python -d domain.local -c Default -u user -p password! -ns 10.129.204.207

# Comprimir output en zip
bloodhound-python -d domain.local -c All -u user -p password! -ns 10.129.204.207 --zip
```
...
echo "10.129.204.207 dc01.domain.local dc01 inlanefreight domain.local" | sudo tee -a /etc/hosts

# Sincronizar tiempo con el DC (obligatorio para Kerberos)
sudo ntpdate 10.129.204.207

# Correr con Kerberos
bloodhound-python -d domain.local -c DCOnly -u user -p password! -ns 10.129.204.207 --kerberos
```
...
```c
bloodhound-python -d domain.local -c All -u user --hashes aad3b435b51404eeaad3b435b51404ee:88ad09182de639ccc6579eb0849751cf -ns 10.129.204.207
```
...
```c
# DC específico
bloodhound-python -d domain.local -c DCOnly -u user -p password! -dc dc01.domain.local -ns 10.129.204.207

# Excluir DCs de enumeración de computadoras
bloodhound-python -d domain.local -c Default -u user -p password! -ns 10.129.204.207 --exclude-dcs

# Número de workers para enumeración de computadoras
bloodhound-python -d domain.local -c Default -u user -p password! -ns 10.129.204.207 -w 5

# Usar solo hosts específicos
bloodhound-python -d domain.local -c ComputerOnly -u user -p password! -ns 10.129.204.207 --computerfile computers.txt

# Verbose
bloodhound-python -d domain.local -c DCOnly -u user -p password! -ns 10.129.204.207 -v
```
...
```c
schtasks /create /tn "SH" /tr "C:\Users\user\Documents\SharpHound.exe -c All --outputdirectory C:\Users\user\Documents" /sc once /st 00:00 /ru INLANEFREIGHT\user /rp password! /f
```
...
echo "10.129.23.176 dc01.domain.local domain.local" | sudo tee -a /etc/hosts

# Recolectar desde Linux directamente
bloodhound-python -d domain.local -c All -u user -p password! -dc dc01.domain.local -ns 10.129.23.176 --dns-tcp --zip
```
