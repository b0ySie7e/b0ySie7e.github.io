...
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local"

# Filtrar solo usuarios
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(objectClass=user)" sAMAccountName

# Filtrar solo grupos
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(objectClass=group)" cn

# Filtrar solo computadoras
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(objectClass=computer)" cn

# Domain Controllers
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=8192)" dNSHostName

# Domain Admins
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(cn=Domain Admins)" member

# Cuentas deshabilitadas
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=2)"

# Contraseñas que no expiran
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=65536)"

# Cuentas con SPN (Kerberoasteables)
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(&(objectClass=user)(servicePrincipalName=*))" sAMAccountName servicePrincipalName

# Cuentas ASREPRoasteables
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=4194304)" sAMAccountName

# Cuentas con Unconstrained Delegation
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=524288)" sAMAccountName memberOf

# Contraseñas en campo description
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(description=*)" description

# Política de contraseñas
ldapsearch -H ldap://10.129.1.207 -x -b "dc=domain,dc=local" -s sub "*" | grep -m 1 -B 10 pwdHistoryLength

# Buscar OU de un usuario específico
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(sAMAccountName=kevin.gregory)" distinguishedName

# Info adicional — campo info y comment
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(info=*)" info
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(comment=*)" comment
```
...
```c
ldapsearch -x -H ldap://<target> -D "" -w "" -b "dc=domain,dc=local"
```
...
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(memberOf=CN=Protected Users,CN=Users,DC=domain,DC=local)" sAMAccountName

# Usuarios en Domain Admins
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(memberOf=CN=Domain Admins,CN=Users,DC=domain,DC=local)" sAMAccountName

# Usuarios en cualquier grupo que contenga "admin" en el nombre
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(memberOf=*admin*)" sAMAccountName
```
...
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(&(userAccountControl:1.2.840.113556.1.4.803:=524288)(memberOf=CN=Protected Users,CN=Users,DC=domain,DC=local))" sAMAccountName

# Usuarios con SPN que sean Domain Admins
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(&(servicePrincipalName=*)(memberOf=CN=Domain Admins,CN=Users,DC=domain,DC=local))" sAMAccountName servicePrincipalName

# Usuarios ASREPRoasteables que tengan description
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(&(userAccountControl:1.2.840.113556.1.4.803:=4194304)(description=*))" sAMAccountName description
```

### Buscar la OU de un usuario específico

```c
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(sAMAccountName=forend)" distinguishedName
```

El DistinguishedName en la respuesta se lee de izquierda a derecha. Por ejemplo `CN=forend,OU=IT Admins,OU=IT,DC=domain,DC=LOCAL` significa que `forend` está en la OU `IT Admins` que a su vez está dentro de la OU `IT`.

### Buscar todos los usuarios dentro de una OU específica

```c
# Usuarios directamente en una OU (sin bajar a sub-OUs)
ldapsearch -x -H ldap://10.129.1.207 -b "OU=IT,DC=domain,DC=local" -s onelevel "(objectClass=user)" sAMAccountName

# Usuarios en una OU y todas sus sub-OUs
ldapsearch -x -H ldap://10.129.1.207 -b "OU=IT,DC=domain,DC=local" -s sub "(objectClass=user)" sAMAccountName
```
...
```c
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(sAMAccountName=forend)" memberOf
```
...
```c
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(cn=Help Desk)" member
```
...
```c
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(objectClass=computer)" operatingSystem operatingSystemVersion dNSHostName
```

### Buscar OUs disponibles en el dominio

```c
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(objectClass=organizationalUnit)" ou distinguishedName
```
...
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(sAMAccountName=john*)" sAMAccountName

# Grupos que contengan "admin" en el nombre
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(cn=*admin*)" cn

# Computadoras que contengan "SQL" en el nombre
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(&(objectClass=computer)(cn=*SQL*))" cn dNSHostName
```

### Contar resultados sin mostrarlos

```c
ldapsearch -x -H ldap://10.129.1.207 -b "dc=domain,dc=local" "(objectClass=user)" sAMAccountName | grep "sAMAccountName:" | wc -l
```
...
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local"

# Opcion 2 - Bind con usuario y contraseña
ldapsearch -x -H ldap://10.129.1.207 -D "admin@domain.local" -w password -b "dc=domain,dc=local"

# Opcion 3 - Bind con usuario y contraseña
ldapsearch -x -H ldap://10.129.1.207 -D "domain\admin" -w password -b "dc=domain,dc=local"

# Opcion 4 - Bind con usuario y contraseña
ldapsearch -x -H ldap://10.129.1.207 -D "CN=hugo Smith,CN=Users,dc=domain,dc=local" -w password -b "dc=domain,dc=local"

```

```c
# Pedir contraseña interactivamente (más seguro)
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -W -b "dc=domain,dc=local"

# LDAPS (SSL/TLS, puerto 636)
ldapsearch -x -H ldaps://10.129.1.207:636 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local"

# Dump completo de todos los usuarios con atributos específicos
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(objectClass=user)" sAMAccountName mail userAccountControl

# Guardar dump completo a archivo
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(objectClass=user)" "*" "+" > all_users.ldif

# Buscar contraseñas en el dump
grep -i "description:" all_users.ldif | grep -i "pass\|pwd\|secret"

# Kerberoasteables
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(&(objectClass=user)(servicePrincipalName=*))" sAMAccountName servicePrincipalName

# ASREPRoasteables
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(&(objectClass=user)(userAccountControl:1.2.840.113556.1.4.803:=4194304))" sAMAccountName

# Cuentas con cifrado reversible (ENCRYPTED_TEXT_PWD_ALLOWED)
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=128)" sAMAccountName

# Smartcard required
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=262144)" sAMAccountName

# Cuentas con PASSWD_NOTREQD
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=32)" sAMAccountName
```
...
python3 windapsearch.py --dc-ip 10.129.1.207 -d domain.local -u domain\\james.cross --unconstrained-computers

# Usuarios privilegiados (todos los grupos admin, recursivo)
python3 windapsearch.py --dc-ip 10.129.1.207 -u domain\\james.cross -PU

# Todos los usuarios
python3 windapsearch.py --dc-ip 10.129.1.207 -u domain\\james.cross -U

# Todas las computadoras
python3 windapsearch.py --dc-ip 10.129.1.207 -u domain\\james.cross -C
...
```c
# Política de contraseñas
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t pass-pols

# Kerberoasteables
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t kerberoast | grep servicePrincipalName:

# ASREPRoasteables
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t asreproast

# Admins del dominio
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t show-admins

# Info de un usuario específico
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t show-user -s "(sAMAccountName=forend)"

# Trusts del dominio
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t trusts

# Ejecutar todo de una vez
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t all

# Con filtro LDAP personalizado
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t search -s "(userAccountControl:1.2.840.113556.1.4.803:=262144)"

# Guardar output a archivo
python3 ldapsearch-ad.py -l 10.129.1.207 -d domain -u james.cross -p Summer2020 -t all -o output.txt
...
conn = Connection(server, 'cn=admin,dc=domain,dc=local', 'password', auto_bind=True)
...
```c
❯ ldapsearch -x -H ldap://10.129.19.102 -b "dc=domain,dc=local" "(userAccountControl:1.2.840.113556.1.4.803:=<Cambiar valor de referncia>)" sAMAccountName
```
...
```c
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(memberOf=CN=Protected Users,CN=Users,DC=domain,DC=local)" sAMAccountName

ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(memberOf=CN=Domain Admins,CN=Users,DC=domain,DC=local)" sAMAccountName
```
...
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(&(userAccountControl:1.2.840.113556.1.4.803:=524288)(memberOf=CN=Protected Users,CN=Users,DC=domain,DC=local))" sAMAccountName memberOf

# Kerberoasteable + Domain Admin
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(&(servicePrincipalName=*)(memberOf=CN=Domain Admins,CN=Users,DC=domain,DC=local))" sAMAccountName servicePrincipalName

# ASREPRoasteable + con description (posible contraseña)
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(&(userAccountControl:1.2.840.113556.1.4.803:=4194304)(description=*))" sAMAccountName description

# Admin (adminCount=1) + PASSWD_NOTREQD
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(&(adminCount=1)(userAccountControl:1.2.840.113556.1.4.803:=32))" sAMAccountName
```

### Buscar la OU de un usuario específico

```c
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(sAMAccountName=forend)" distinguishedName
```

El DistinguishedName se lee de izquierda a derecha: primero el CN del usuario, luego las OUs de más específica a más general, al final el dominio. `CN=forend,OU=IT Admins,OU=IT,DC=domain,DC=LOCAL` = usuario en OU `IT Admins` dentro de la OU `IT`.

### Buscar todos los usuarios dentro de una OU

```c
# Solo en esa OU sin bajar a sub-OUs
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "OU=IT,DC=domain,DC=local" -s onelevel "(objectClass=user)" sAMAccountName

# En esa OU y todas sus sub-OUs
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "OU=IT,DC=domain,DC=local" -s sub "(objectClass=user)" sAMAccountName
```

### Buscar todos los grupos a los que pertenece un usuario

```c
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(sAMAccountName=forend)" memberOf
```

### Buscar miembros de un grupo

```c
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(cn=Help Desk)" member
```

### Buscar OUs disponibles en el dominio

```c
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(objectClass=organizationalUnit)" ou distinguishedName
```
...
```c
# Usuarios cuyo nombre empiece con "john"
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(sAMAccountName=john*)" sAMAccountName

# Grupos que contengan "admin"
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(cn=*admin*)" cn

# Computadoras con SQL en el nombre
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(&(objectClass=computer)(cn=*SQL*))" cn dNSHostName
```

### Contar resultados

```c
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(objectClass=user)" sAMAccountName | grep "sAMAccountName:" | wc -l
```
...
```c
ldapsearch -x -H ldap://10.129.1.207 -D "cn=admin,dc=domain,dc=local" -w password -b "dc=domain,dc=local" "(sAMAccountName=kevin.gregory)" distinguishedName
```
