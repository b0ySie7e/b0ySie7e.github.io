...
Si el resultado muestra que el Administrator de `DOMAIN.LOCAL` es miembro del grupo Administrators de `FREIGHTLOGISTICS.LOCAL`, puedes autenticarte directamente al DC del bosque remoto con esa cuenta.

### Conectarse al DC del bosque remoto con esas credenciales

```c
Enter-PSSession -ComputerName ACADEMY-EA-DC03.FREIGHTLOGISTICS.LOCAL -Credential DOMAIN\administrator
```
...
```c
GetUserSPNs.py -target-domain FREIGHTLOGISTICS.LOCAL DOMAIN.LOCAL/wley
```
...
Para encontrar usuarios de un dominio que son miembros de grupos en otro dominio (como el Administrator de DOMAIN.LOCAL siendo miembro del grupo Administrators de FREIGHTLOGISTICS.LOCAL), se puede usar bloodhound-python para recolectar datos de ambos dominios y visualizarlo en BloodHound.
...
Nota que aquí el usuario se especifica como `forend@DOMAIN.local` con el dominio completo porque te estás autenticando al dominio remoto con credenciales del dominio local.

### Visualizar en BloodHound

Subir ambos zips al GUI de BloodHound, ir a la pestaña Analysis y usar la query `Users with Foreign Domain Group Membership`, seleccionar `DOMAIN.LOCAL` como dominio origen. Muestra qué usuarios de ese dominio son miembros de grupos en otros dominios del bosque.
