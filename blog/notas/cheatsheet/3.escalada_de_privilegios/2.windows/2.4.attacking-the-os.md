...
## Enumeración de Versión del Sistema Operativo
...
Get-ComputerInfo | Select-Object OsName, OsVersion, OsBuildNumber, OsArchitecture, OsServicePackMajorVersion
 
# Alternativa con CimInstance
Get-CimInstance -ClassName Win32_OperatingSystem | 
    Select-Object Caption, Version, BuildNumber, OSArchitecture, ServicePackMajorVersion, LastBootUpTime
 
# Con systeminfo (más completo)
systeminfo | findstr /i "OS Name\|OS Version\|System Type\|Build"
```
 
**Output típico y qué significa:**
 
```
OS Name:    Microsoft Windows 10 Pro
OS Version: 10.0.19041 Build 19041    ← Build number es clave
System Type: x64-based PC
```
 
**Tabla de builds importantes:**
...
Windows 10:
  ...
  20H2  → Build 19042
  21H1  → Build 19043
  22H2  → Build 19045  (más común actualmente)
...
Cuanto más antiguo el build, más CVEs disponibles.
 
---
 
## Enumeración de Parches Instalados
 
### Ver parches instalados
 
```powershell
# Todos los hotfixes instalados
...
# Cantidad total de parches instalados
(Get-HotFix).Count
 
# Ver si un parche específico está instalado (buscar por KB)
Get-HotFix -Id KB5012345
# Si no devuelve nada = parche NO instalado = sistema vulnerable
...
**Buscar parches faltantes críticos:**
 
```powershell
# Lista de parches instalados
$installedPatches = (Get-HotFix).HotFixID
 
# Verificar si un parche específico está ausente
$criticalPatch = "KB5012345"
if ($installedPatches -notcontains $criticalPatch) {
    Write-Host "PARCHE FALTANTE: $criticalPatch - Sistema posiblemente vulnerable" -ForegroundColor Red
} else {
    Write-Host "Parche $criticalPatch instalado" -ForegroundColor Green
}
```
...
### Ver aplicaciones y sus versiones
...
# Aplicaciones 32-bit (en sistema 64-bit)
Get-ItemProperty "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*" | 
    Select-Object DisplayName, DisplayVersion, Publisher, InstallDate |
    Where-Object {$_.DisplayName -ne $null} |
    Sort-Object DisplayName
 
# Ambas en una sola query
$apps = @(
    "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*",
    "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*"
)
Get-ItemProperty $apps | 
    Select-Object DisplayName, DisplayVersion, Publisher, InstallDate |
    Where-Object {$_.DisplayName -ne $null} |
...
**Buscar versiones vulnerables de aplicaciones específicas:**
 
```powershell
# Buscar versión de una aplicación concreta
Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*" | 
    Where-Object {$_.DisplayName -like "*NombreApp*"} | 
    Select-Object DisplayName, DisplayVersion, InstallLocation
 
# Buscar aplicaciones por publisher
Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*" | 
    Where-Object {$_.Publisher -like "*NombreVendor*"} | 
    Select-Object DisplayName, DisplayVersion
```
 
---
 
## Enumeración de Procesos y Versiones en Ejecución
 
```powershell
# Ver todos los procesos con ruta del ejecutable
Get-Process | Select-Object Name, Id, Path, FileVersion, ProductVersion | 
    Where-Object {$_.Path -ne $null} | Format-Table -AutoSize
 
# Versión de un proceso específico
Get-Process -Name "NombreApp" | Select-Object Name, FileVersion, Path
 
# Ver información de versión de un ejecutable
(Get-Item "C:\ruta\al\app.exe").VersionInfo
 
# Alternativa
[System.Diagnostics.FileVersionInfo]::GetVersionInfo("C:\ruta\al\app.exe")
```
 
---
 
## Enumeración de Drivers y Kernel
 
```powershell
# Ver drivers del sistema (pueden tener vulnerabilidades propias)
Get-WindowsDriver -Online | Select-Object Driver, Version, ProviderName, Date | 
    Sort-Object Date -Descending | Format-Table -AutoSize
 
# Drivers de terceros (no Microsoft)
Get-WindowsDriver -Online | 
    Where-Object {$_.ProviderName -notlike "*Microsoft*"} | 
    Select-Object Driver, Version, ProviderName, Date
 
# Con driverquery (cmd)
driverquery /v /fo csv | ConvertFrom-Csv | 
    Select-Object "Module Name", "Display Name", "Driver Type", "State" | 
    Format-Table -AutoSize
 
# Drivers firmados vs sin firma (sin firma = más vulnerables)
driverquery /si
```
 
---
 
## Flujo Completo: Recopilar para WES-NG
 
```powershell
# 1. Recopilar systeminfo completo
systeminfo > C:\temp\sysinfo.txt
 
# 2. Recopilar parches instalados
Get-HotFix | Select-Object HotFixID | Out-File -Append C:\temp\sysinfo.txt
 
# 3. Transferir a Kali
# (desde Kali)
scp usuario@192.168.1.100:C:\temp\sysinfo.txt .
 
# 4. Analizar con WES-NG
python3 wes.py sysinfo.txt
 
# 5. Filtrar solo LPE (Local Privilege Escalation)
python3 wes.py sysinfo.txt | grep -i "privilege\|local\|elevation"
 
# 6. Filtrar por severidad
python3 wes.py sysinfo.txt --impact "Elevation of Privilege"
 
# 7. Excluir exploits sin PoC disponible
