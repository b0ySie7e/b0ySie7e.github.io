...
### Identificar qué usuario ejecuta cada tarea

Esta es la parte clave — saber el usuario te dice si la tarea es explotable para escalar privilegios.

#### El formato de /etc/crontab lo dice directamente

`/etc/crontab` y los archivos en `/etc/cron.d/` tienen una columna extra que los crontabs de usuario no tienen — el **usuario**:

```
# MIN  HORA  DIA  MES  DIASEM  USUARIO    COMANDO
  *    *     *    *    *       root       /usr/local/bin/backup.sh
  */5  *     *    *    *       www-data   /opt/scripts/clean.sh
  0    2     *    *    *       john       tar -czf /tmp/backup.tar.gz /home
#                               ^^^^
#                               Esta columna es el usuario que ejecuta el comando
```

> Si dice `root` → esa tarea corre como root → si puedes modificar lo que ejecuta, eres root.

#### Los directorios cron.daily/hourly/etc no tienen columna de usuario

Los scripts dentro de `/etc/cron.daily/`, `/etc/cron.hourly/`, etc. los lanza el demonio cron usando el usuario definido en `/etc/crontab` para esa sección. Para verlo:

```bash
# Busca las líneas que llaman a run-parts
...
#               ^^^^
#               Ese "root" ejecuta TODOS los scripts de ese directorio
```

#### Crontabs de usuarios — el dueño es el que lo ejecuta

```bash
# Ver crontabs de todos los usuarios del sistema de una vez
...
> Las tareas en `crontab -l` siempre corren como el usuario dueño del crontab.
> Si `john` tiene una tarea en su crontab → corre como `john`.

#### Desde los logs — ver quién ejecutó qué y cuándo

```bash
# Ver usuario + comando en tiempo real
grep "CRON" /var/log/syslog | grep "CMD"

# Solo tareas de root
grep "CRON" /var/log/syslog | grep "(root)"

# Las últimas 50 ejecuciones
grep "CRON" /var/log/syslog | tail -50
```

**Output típico:**
```
Jul 27 03:00:01 host CRON[1234]: (root) CMD (/usr/local/bin/backup.sh)
#                                  ^^^^
#                                  El usuario está entre paréntesis
```

#### pspy — ver procesos en tiempo real sin ser root

`pspy` es la herramienta más útil en labs. Monitorea procesos nuevos sin privilegios — detecta crons que no aparecen en ningún archivo de configuración.

```bash
# Descargar en Kali y subir al target
...
**Output de pspy:**
```
2026/07/27 03:00:01 CMD: UID=0    PID=1234   | /bin/sh -c /usr/local/bin/backup.sh
#                             ^^^
#                             UID=0 → root
#                             UID=1000 → usuario normal
```

> Deja pspy corriendo 5-10 minutos para capturar tareas que corren cada 1, 5 o 10 minutos.

#### Tabla resumen — dónde encontrar el usuario según el tipo de cron

| Dónde está la tarea | Cómo ver el usuario |
|---------------------|---------------------|
| `/etc/crontab` | Columna 6 del archivo |
| `/etc/cron.d/*` | Columna 6 del archivo |
| `/etc/cron.daily/` | Ver `grep "run-parts" /etc/crontab` — casi siempre root |
| `/etc/cron.hourly/` | Igual que cron.daily |
| `crontab -l` de un usuario | El dueño del crontab siempre |
| Logs `/var/log/syslog` | Campo entre paréntesis: `(root) CMD` |
| pspy | Campo `UID=` en cada línea |

---

### Verificar si la tarea es explotable

Una vez que sabes el usuario, verifica si puedes modificar algo que esa tarea usa:

```bash
# ¿Puedes escribir el script que ejecuta?
ls -la /usr/local/bin/backup.sh
stat /usr/local/bin/backup.sh

# ¿Puedes escribir en el directorio donde está?
ls -la /usr/local/bin/

# ¿El script llama binarios sin ruta absoluta? → PATH hijacking
cat /usr/local/bin/backup.sh
# Si llama a "python" en vez de "/usr/bin/python" → vulnerable

# ¿Tiene wildcards * en tar, cp, chmod?
